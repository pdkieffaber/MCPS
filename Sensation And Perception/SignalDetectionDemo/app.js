...
function createPlanes(trial) {
  const arr = [];
  currentTargetIndex = trial.signalPresent
  ...
  for (let i = 0; i < N_PLANES; i++) {
    ...
    const speed = i === currentTargetIndex ? trial.targetSpeed : trial.baseSpeed;
...
function showBlockInstructions(block) {
  ...
  if (block === 1) {
    ...
    lines = [
      "Block 3: Response Bias",
      "",
      "Missing a faster airplane is costly.",
      "If unsure, lean toward saying signal present.",
      "",
      "Press F = one airplane was faster",
      "Press J = all airplanes were the same",
      "",
...
function getTargetPosition() {
  if (currentTargetIndex < 0 || !planes[currentTargetIndex]) {
    return { x: 0, y: 0 };
  }
  return {
    x: planes[currentTargetIndex].x,
    y: planes[currentTargetIndex].y
  };
}

function drawExplosionFrame(centerX, centerY, progress, isBig) {
  clearScene();
  drawFieldBoundary();

  const x = CX + centerX;
  const y = CY + centerY;
  const baseRadius = isBig ? 28 : 16;
  const maxRadius = isBig ? 150 : 85;
  ...
  core.addColorStop(0.0, `rgba(255,255,220,${0.95 - progress * 0.45})`);
  core.addColorStop(0.25, `rgba(255,200,60,${0.90 - progress * 0.35})`);
  core.addColorStop(0.55, `rgba(255,120,20,${0.70 - progress * 0.35})`);
  core.addColorStop(1.0, `rgba(255,40,0,0)`);
  ctx.fillStyle = core;
  ctx.beginPath();
  ctx.arc(x, y, radius, 0, Math.PI * 2);
  ctx.fill();

  const ringAlpha = Math.max(0, 0.8 - progress * 0.8);
  ctx.strokeStyle = `rgba(255,220,120,${ringAlpha})`;
  ctx.lineWidth = isBig ? 5 : 3;
  ctx.beginPath();
  ctx.arc(x, y, radius * 0.75, 0, Math.PI * 2);
  ctx.stroke();

  const rayCount = isBig ? 18 : 12;
  const inner = radius * 0.2;
  const outer = radius * (isBig ? 1.1 : 0.95);
  ctx.strokeStyle = `rgba(255,180,60,${Math.max(0, 0.9 - progress)})`;
  ctx.lineWidth = isBig ? 3 : 2;
  for (let i = 0; i < rayCount; i++) {
    const angle = (Math.PI * 2 * i) / rayCount + progress * 0.7;
...
function playExplosion(x, y, isBig, onDone) {
  feedbackPlaying = true;
  const start = performance.now();

  function frame(now) {
    const progress = Math.min(1, (now - start) / EXPLOSION_DURATION_MS);
    drawExplosionFrame(x, y, progress, isBig);
...
function submitResponse(key) {
  if ((!running && !awaitingResponse) || !currentTrial || feedbackPlaying) return;
  ...
  const isHit = currentTrial.signalPresent && responseLabel === "signal";
  const isMiss = currentTrial.signalPresent && responseLabel !== "signal";
  ...
  results.push({
    ...
    feedbackType: isHit ? "hit_explosion" : isMiss ? "miss_explosion" : "none"
  });
  ...
  if (isHit) {
    playExplosion(targetPosition.x, targetPosition.y, false, queueNextTrial);
    return;
  }

  if (isMiss) {
    playExplosion(0, 0, true, queueNextTrial);
    return;
  }
...
document.addEventListener("keydown", (e) => {
  ...
  if (!waitingForBlockStart && !feedbackPlaying) {
