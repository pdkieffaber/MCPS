---
title: "From Data to Discovery: Methods & Computing for the Psychological Sciences"
...
```{r, 2D Clusters, message = FALSE, warning = FALSE, echo=FALSE}
library(ggplot2)
library(dplyr)
...
```{r 3D Clusters, message = FALSE, warning = FALSE, echo=FALSE}
library(plotly)
...
We define the total within-cluster sum of squares (WSS) as follows:
$$
WSS = \sum_{k=1}^{k}W(C_k) = \sum_{k=1}^{k}\sum_{x_i \in C_k}{(x_i - u_k)^2}
$$
The total within-cluster sum of square measures the compactness (i.e goodness) of the clustering and we want it to be as small as possible.

..but first we have to find the best $K$.
...
1) The Elbow Method: This method plots the within-cluster sum of squares (WSS) against different values of K. The WCSS measures the compactness of the clusters. As K increases, WSS will always decrease. You look for the "elbow" in the plot—the point where adding another cluster doesn't "significantly" reduce the WCSS.
...
```
Interpretation: The plot shows a very clear "elbow" at K=4. After this point, adding more clusters doesn't significantly reduce the within-cluster sum of squares (WSS).
...
km.res <- kmeans(df, centers = 4, nstart = 25, algorithm = c("Hartigan-Wong"))
...
**The Crucial Step:** "Linkage" Defines Cluster Distance. The rule you use to define "cluster-to-cluster" distance is called the linkage method. This is the method argument in the `hclust()` function, and it's the most important choice you'll make. There are several common types:
  ...
  - Ward's Method (ward.D2): This is the most popular and generally most robust method. It's different from the others. It doesn't just use a simple distance. Instead, it asks: "If I merge these two clusters, what will be the new total within-cluster variance (WSS)?" The algorithm merges the two clusters that result in the minimum increase in the total WCSS. By doing this at every step, it tends to produce very compact, spherical, and evenly-sized clusters. It's conceptually very similar to K-Means (which also tries to minimize WCSS), making it a powerful default choice.
