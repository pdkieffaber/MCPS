...
\begin{figure}[h]
\begin{tcolorbox}[halign=center, drop shadow, title=Binary \& Weighted Adjacency Matrices ,colback=white,colframe=WMgreen,
  colbacktitle=WMgreen,]
\begin{tikzpicture}[scale=0.8]
  % Common styles
  \tikzset{
    title/.style   = {anchor=south, font=\bfseries},
    collabel/.style= {anchor=south},
    rowlabel/.style= {anchor=east}
  }

  % ---------- Binary Adjacency Matrix (LEFT) ----------
  \begin{scope}[shift={(-6.25,0)}]
    % Title centered over the 4x4 grid (center x = 2)
    \node[title] at (2,5.2) {Binary Adjacency Matrix};
    ...
    \foreach \x/\l in {0/A,1/B,2/C,3/D} {
      \node[collabel] at (\x+0.5,4.25) {\l};
    }
    % Row labels (left of grid; top-to-bottom A..D)
    \foreach \y/\l in {3/A,2/B,1/C,0/D} {
      \node[rowlabel] at (-0.25,\y+0.5) {\l};
    }
  ...
  % ---------- Weighted Adjacency Matrix (RIGHT, pushed further right) ----------
  \begin{scope}[shift={(2.5,0)}]
    % Title centered over the 4x4 grid (center x = 2)
    \node[title] at (2,5.2) {Weighted Adjacency Matrix};
  ...
  % ---------- Binary Edge List (centered under left matrix) ----------
  \begin{scope}[shift={(-6.25,-5)}]
    \node[title] at (2,4) {Binary Edge List};
    \node[align=center, anchor=north] at (2,3.2) {
      A -- B\\
      A -- C\\
      B -- D\\
      C -- D
  ...
  % ---------- Weighted Edge List (centered under right matrix) ----------
  \begin{scope}[shift={(2.5,-5)}]
    \node[title] at (2,4) {Weighted Edge List};
    \node[align=center, anchor=north] at (2,3.2) {
      A -- B (0.8)\\
      A -- C (0.4)\\
      B -- D (0.6)\\
  ...
  \end{scope}
\end{tikzpicture}
 \captionof{figure}{Illustration of binary (left) and weighted (right) adjacency matrices.}
  \label{fig:MatrixTypes}
 \end{tcolorbox}
 \end{figure}
