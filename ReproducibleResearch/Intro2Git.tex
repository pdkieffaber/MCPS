...
\subsubsection{MacOS Option 1: Command Line Tools (Recommended)}
\begin{enumerate}
...
\subsubsection{MacOS Option 2: Homebrew}
\begin{enumerate}
\item Open Terminal.
...
In your terminal (Git Bash, Command Prompt, or macOS Terminal), run:
\begin{terminal}
...
\item On macOS or Linux
	\begin{itemize}
		...
		\begin{terminal}
		\terminalinput{cd \textasciitilde /Documents/Projects}
		\end{terminal}
...
\item{On Windows (Using Command Prompt)}
	...
	Navigate to another directory using the ``cd'' (change directory) command:
	\begin{terminal}
	\terminalinput{cd C:\textbackslash{}Users\textbackslash{}YourUsername\textbackslash{}Documents\textbackslash{}Projects}
	\end{terminal}
...
\section{Assignment}
\begin{enumerate}
\item Create (if you haven't already) an account on GitHub (\weblink{https://github.com}) and log into your account.

\item On the GitHub website, create a new public repository named ``ReproducibleResearch\_FIRST\_LAST'' (replace FIRST and LAST with your first and last names respectively).

	\begin{itemize}
	\item Click the Repositories tab
	\item Click the ``New'' button to create a new repository
	\end{itemize}
\item Clone your new repository to your computer:
	\begin{terminal}
	\terminalinput{cd SOME\_FOLDER}\\
	\terminalinput{git clone LINK\_TO\_YOUR\_REPOSITORY}\\
	\terminalinput{cd ReproducibleResearch\_FIRST\_LAST}
	\end{terminal}
	
\item Create (or edit) the README.txt file, adding the line, ``\textit{First Last}-Spring, 2026'' (replace \textit{First} with your first name and \textit{Last} with your last name)

\item Commit changes and push them back to GitHub:
	\begin{terminal}
	\terminalinput{git add .}\\
...
\item Visit the spreadsheet \href{https://docs.google.com/spreadsheets/d/1L9bVyOK4S6QU2VVjuUZQyv07SR5BpAGEGkK9tA4zqBA/edit?usp=sharing}{HERE} and add your name and the link to your updated repository.\\
\footnotesize{(https://docs.google.com/spreadsheets/d/1L9bVyOK4S6QU2VVjuUZQyv07SR5BpAGEGkK9tA4zqBA/edit?usp=sharing)}
	
\end{enumerate}

\printglossary[type=reproducibleresearch,style=twocolumn]
%\printglossary
%\glsresetall
\newpage
