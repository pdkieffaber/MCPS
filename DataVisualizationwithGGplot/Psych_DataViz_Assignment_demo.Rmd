...
```{r p1}
Newdata <- bfi %>% mutate(Agree = rowMeans(select(.,A1:A5),na.rm=TRUE))

ggplot(Newdata, aes(x=Agree)) +
  geom_density(fill = "blue", alpha = 0.6) +
  labs(title = "Distribution of Agreeableness", y = "Count") + 
  theme_minimal()
```
...
```{r p2}
bfi %>% mutate(Neuroticism=rowMeans(select(.,N1:N5),na.rm=TRUE)) %>% 
  mutate(gender=factor(gender,levels=c(1,2),labels=c('Male','Female'))) %>% 
  ggplot(aes(x=gender,y=Neuroticism,fill=gender)) +
     geom_boxplot() +
     labs(title = "Boxplot of Neuroticism by Gender", y = "Neuroticism", x="Gender") + 
     theme_minimal() +
...
```{r p3}
bfi %>% mutate(Conscientiousness=rowMeans(select(.,C1:C5),na.rm=TRUE)) %>%
  group_by(education) %>% 
  summarise(AvgConscientiousness=mean(Conscientiousness, na.rm=TRUE),
            sd_c = sd(Conscientiousness),
            n_c = n(),
            se = sd_c / sqrt(n_c),
            ci = 1.96 * se,
            lower = AvgConscientiousness - ci, 
            upper = AvgConscientiousness + ci,
            .groups = "drop"
            ) %>% 
  ggplot(aes(x=education,y=AvgConscientiousness,fill=education)) +
     geom_col() +
     geom_errorbar(aes(ymin = AvgConscientiousness-sd_c, ymax = AvgConscientiousness+sd_c), 
                   width = 0.2,
                   color = "black"
     ) +
     labs(title = "Column Plot of Mean Conscientiousness by Education level", y = "Conscientiousness", x="Education") + 
     theme_minimal() +
     theme(legend.position = "none") +
     scale_x_discrete(
      limits = c("1", "2", "3","4","5"), # limits controls order of factors
      labels = c("High School", "Finished HS", "Some College", "BA/BS","Graduade Degree")) 
...
bfi%>%mutate(Conscientiousness=rowMeans(select(.,C1:C5),na.rm=TRUE))%>%
  group_by(education)%>%
  summarise(mean_c_score=mean(Conscientiousness,na.rm=TRUE),
            sd_c = sd(Conscientiousness),
            n=n(),
            se=sd_c/sqrt(n),
            ci=1.96*se,
            lower=mean_c_score-ci,
            upper=mean_c_score+ci)%>%
   ggplot(aes(x=education,y=mean_c_score,fill=education))+
   geom_col()+
   geom_errorbar(aes(ymin=lower,ymax=upper,width=0.2,color="black"))+
   labs(title="Conscientiousness Across Education",subtitle="An Average",x="Education Level",y="Average Conscientiousness")+
   theme_minimal()+
   theme(legend.position="none")+
   scale_x_discrete(
    limits = c("5","4","3", "2", "1"), 
    labels = c("Graduate Degree", "College Grad", "Some College","HS Grad", "HS"))
...
**Goal:** Examine the relationship (i.e., correlation) between any **two continuous variables** of your choice.  
...
**Goal:** Use a boxplot to examine how the mean of one continuous variable varies across levels of a grouping variable (e.g., mean petal length across plant species).
...
**Goal:** Use the same data you used for #15 to create a Boxplot.  
...
**Goal:** Use your own data to answer a question you have about your data by producing **1–2 plots**. 

Your "story" should include:

* At least one continuous variable and two categorical variables with 2 or more levels

* Publication quality plots of the data distributions (use aes() and/or faceting)

* Publication quality plots of the data summaries with estimates of uncertainty (e.g., boxplot or bar chart with error bars, geom_col)

* All appropriate labels

* Write a short caption (2–4 sentences) explaining what your graphs show.
