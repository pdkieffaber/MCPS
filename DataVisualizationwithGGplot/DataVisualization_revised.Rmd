...
> **Prerequisites (quick check):** This chapter assumes you can:
> - load packages with `library(...)` and install them if needed,
> - recognize a data frame/tibble and identify what one row represents,
> - do very basic filtering/selecting/summarizing when preparing data for plots,
> - interpret `NA` and common warnings about missing rows in plots.
...
# Mapping vs. setting aesthetics in ggplot2
> **Mini-exercise: mapping vs. setting**
> 1. Modify a scatterplot so all points are **blue** (no legend).
> 2. Modify the same plot so points are colored by **gender** (with a legend).
> 3. In one sentence, explain why `aes(color = "blue")` produces a legend but `geom_point(color = "blue")` does not.

```{r mapping_vs_setting_exercise, eval=FALSE}
# 1) Set (no legend)
ggplot(starwars_clean, aes(height, mass)) +
  geom_point(color = "blue")

# 2) Map (legend)
ggplot(starwars_clean, aes(height, mass, color = gender)) +
  geom_point()
...
In ggplot2, aesthetics are visual properties such as x, y, color, fill, size, shape, and alpha. A key rule is: if an aesthetic is inside aes(), you are mapping it to a variable in your data; if it is outside aes(), you are setting it to a constant value. Mapping is used when you want the plot to encode information (e.g., different groups shown with different colors). Setting is used when you want a fixed style (e.g., all points blue). For example, aes(color = condition) tells ggplot2 to color points by the values of condition and it will automatically create a legend. In contrast, color = "blue" makes all points blue and does not create a legend.

For example, a mapping, where color represents a variable (creates a legend) would require a command like:

```{r, eval=FALSE}
ggplot(psych_data, aes(x = stress, y = rt, color = condition)) +
  geom_point()
```

However, setting the point color to a constant style (blue; no legend) would look like this:

```{r, eval=FALSE}
ggplot(psych_data, aes(x = stress, y = rt)) +
  geom_point(color = "blue")
```

**Common confusion:** writing aes(color = "blue") will not do what you intend because it maps the literal text "blue" as if it were a category, and you’ll often get a legend with a single level.

# “+” layering: building a plot one layer at a time

You my have already noticed this, but ggplot2 plots are built by adding layers with the + operator. You typically start with a base ggplot() call that defines the dataset and the core mappings, then you add layers such as geoms, scales, and labels. Each geom_*() adds a visual layer (points, lines, bars, etc.), and functions like labs() add titles and axis labels. A practical rule is to treat the plot like a sentence: start with the subject (ggplot(data, aes(...))), then add clauses (+ geom_point() + labs(...) + theme(...)).

A common syntax pitfall is where you place the +. In R, it is safest to put + at the end of the line so R knows the command continues.

# Recommended formatting: + at the end of the line
ggplot(psych_data, aes(x = stress, y = rt_ms, color = condition)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE) +
  labs(
    title = "Stress and Reaction Time by Condition",
    x = "Stress (1–7)",
    y = "Reaction time (ms)"
  )


If you put + in the wrong place, you may see errors like “unexpected ‘+’” or “Cannot add ggproto objects together.” When that happens, check the line breaks first—most layering errors are formatting issues, not conceptual ones.
...
> **Rule of thumb (scatterplots):**
> - Use when both variables are numeric and you care about **relationships**.
> - Add `alpha = ...` when points overlap heavily.
> - If you map `color` to a variable, you will get a legend; if you set `color = "blue"` outside `aes()`, you will not.
> - Always check for and handle extreme outliers that can distort the scale.
...
> **Rule of thumb (bars):**
> - Use `geom_bar()` for **counts** from raw data (it counts rows for you).
> - Use `geom_col()` when you already computed a summary value (means/totals).
> - If you want side-by-side groups, use `position = "dodge"`; if you want stacked, omit it.
...
### Ordering categories for clearer plots

By default, ggplot2 orders categorical axes alphabetically. Often, a more informative choice is to order categories by a meaningful statistic (e.g., frequency or median).

```{r factor_ordering_example, eval=FALSE}
# Example: order species by median height (requires forcats)
library(forcats)
...
starwars_topspecies %>%
  ...
  geom_boxplot() +
  coord_flip() +
  labs(title = "Heights by Species (ordered by median height)", x = "", y = "Height") +
  theme_apa() +
  theme(legend.position = "none")

# Base R alternative (reorder):
# starwars_topspecies$species <- reorder(starwars_topspecies$species, starwars_topspecies$height, FUN = median)
```

## Histograms and Density Plots (geom_histogram and geom_density):
> **Rule of thumb (histograms/density):**
> - Histograms show **distribution shape**; choose `binwidth` deliberately.
> - A warning about “removed rows” typically means missing values—decide whether to filter them or use `na.rm = TRUE` where supported.
> - Density plots are smoother but can hide small-sample irregularities.
...
> **Rule of thumb (boxplots):**
> - Great for comparing distributions across groups (median, IQR, potential outliers).
> - Boxplots can hide multimodality—consider adding jitter or a violin plot overlay for texture.
> - If group order matters, reorder the factor so the plot tells the story clearly.
...
### A publication-ready plot skeleton (template)

Use this as a reusable starting point. It forces you to specify the core “communication” pieces (labels, caption, and theme) every time.

```{r publication_ready_skeleton, eval=FALSE}
ggplot(DATA, aes(x = X_VAR, y = Y_VAR)) +
  GEOM_LAYER +
  labs(
    title = "Clear, specific title",
    subtitle = "Optional: brief context (sample, timeframe, condition)",
    x = "X axis label (include units if relevant)",
    y = "Y axis label (include units if relevant)",
    caption = "Source: ... | Notes: exclusions/transformations ..."
  ) +
  theme_apa()
```
...
## Color best practices (accessibility)

A few guidelines will make your figures easier to interpret and more accessible:

- Prefer **colorblind-friendly** palettes for categorical variables (e.g., ColorBrewer `Set2`) and **perceptually uniform** palettes for continuous variables (e.g., `viridis`).
- Avoid red/green pairings when possible.
- If color encodes meaning, ensure the legend labels are explicit and match your research language.
- Use transparency (`alpha`) and/or smaller points when overplotting is likely.
...
> **Rule of thumb (faceting):**
> - Use `facet_wrap()` to split by one variable; `facet_grid()` to split by two variables.
> - Keep axes consistent when comparisons across panels matter.
> - If many panels become tiny, consider filtering to key groups or summarizing first.
...
# (Filter to the two gender categories used in the manual color scale to avoid warnings.)
starwars_gender <- starwars_clean %>%
  filter(!is.na(gender), gender %in% c("feminine", "masculine"))

p <- ggplot(starwars_gender, aes(x = height, y = mass, color = gender)) +
  geom_point() +
  scale_color_manual(values = c("feminine" = "black", "masculine" = "darkgray")) +
  labs(title = "Mass vs. Height (Colored by Gender)") +
  theme_apa()
...
```
### Violin plots
> **Rule of thumb (violin plots):**
> - Use when you want the **shape** of the distribution *and* group comparisons.
> - Pair with jitter for transparency about sample size and clustering.
> - Interpret thin tails cautiously when the group has few observations.
...
ggplot(us_map, aes(long, lat, group = group, fill = stress_index)) +
  geom_polygon(color = "black", linewidth = 0.2) + # Border between states
...
ggplot(bfi_person, aes(Extraversion, Neuroticism)) +
  ...
  scale_fill_viridis_d(name = "Density Level") +
  labs(
    title = "Filled 2D Density Plot of Extraversion vs. Neuroticism (BFI)",
    x = "Extraversion (mean score)",
    y = "Neuroticism (mean score)"
  ) +
  theme_minimal(base_size = 14)
```
...
1. When should you use `geom_bar()` instead of `geom_col()`?
2. What is the difference between *mapping* an aesthetic inside `aes()` and *setting* it outside `aes()`?
3. What does it mean when ggplot prints a warning like “Removed X rows containing missing values”?
4. When would faceting be preferable to coloring points by a group variable?
5. Name one reason you might prefer a `viridis` palette for continuous variables.

# Mini-capstone: three descriptive plots

Choose **one** dataset used in this chapter (or another dataset you have permission to use). Create:

1. **A distribution plot** (histogram or density) for a numeric variable.  
2. **A relationship plot** (scatterplot) between two numeric variables (consider `alpha`).  
3. **A grouped comparison plot** (boxplot or violin plot) comparing a numeric variable across categories (consider reordering categories).

Each plot must include:
- a clear title and axis labels via `labs()`,
- an appropriate theme (e.g., `theme_apa()`),
- and a brief caption noting any filtering or missing-data handling.
