...
-   Measures how much Y changes, on average, for each one-unit change in X. $$\beta_1 = \frac{\text{Cov}(X, Y)}{\text{Var}(X)} = \frac{\frac{\sum_{i=1}^{n} (X_i - \bar{X})(Y_i - \bar{Y})}{n - 1}}{\frac{\sum_{i=1}^{n} (X_i - \bar{X})^2}{n - 1}} $$

2)  The Intercept ($\beta_0$):

-   Calculated to ensure the line passes through the "center of mass" of the data: $(\bar{X}, \bar{Y})$. $$\beta_0 = \bar{Y} - \beta_1\bar{X}$$
...
model_aov <- lm(Petal.Length ~ Species, data = iris)
```
...
model_2x2 <- lm(len ~ supp * dose, data = ToothGrowth)
